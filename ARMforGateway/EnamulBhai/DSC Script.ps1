...
configuration VMConfiguration
{
    ...
    # $path1 = Join-Path $folder "PackageTmp";
    ...
    node "wapi"
    {
        Connect-AzAccount -Identity -AccountId '42fad3da-2813-4ea6-a084-c0c54c7ffe48'

        $userName1 = Get-AzKeyVaultSecret -VaultName 'testhuluKV' -Name 'username1' -AsPlainText
        $userPassword1 = Get-AzKeyVaultSecret -VaultName 'testhuluKV' -Name 'pw1' -AsPlainText
        $secStringPassword1 = ConvertTo-SecureString $userPassword1 -AsPlainText -Force
        $credObject1 = New-Object System.Management.Automation.PSCredential ($userName1, $secStringPassword1)

        $userName2 = Get-AzKeyVaultSecret -VaultName 'testhuluKV' -Name 'username2' -AsPlainText
        $userPassword2 = Get-AzKeyVaultSecret -VaultName 'testhuluKV' -Name 'pw2' -AsPlainText
        $secStringPassword2 = ConvertTo-SecureString $userPassword2 -AsPlainText -Force
        $credObject2 = New-Object System.Management.Automation.PSCredential ($userName2, $secStringPassword2)

        $monitorlocation = Get-AzureStorageBlobContent -Container "artifactpipeline" -Blob "PackageTmp" -Destination $path1
        ...
        # ### Copy Monitoring Solution ###
        File DirectoryCopy
        {
           Ensure = "Present" # Ensure the directory is Present on the target node.
           Type = "Directory" # The default is File.
           Recurse = $true # Recursively copy all subdirectories.
           SourcePath = $monitorlocation
           DestinationPath = "C:\inetpub\wwwroot\PackageTmp\"
           DependsOn = '[WindowsFeature]IIS'
